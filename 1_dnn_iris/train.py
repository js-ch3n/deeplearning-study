...
# 数据加载
# ===========================
def load_data(csv_path, batch_size=16, train_ratio=0.8, seed=42):
...
def test_from_file(model_path, config_path="config.yaml"):
    ...
    csv_path = data_cfg.get("csv_path", "data/iris.csv") if isinstance(data_cfg, dict) else str(data_cfg)
    train_ratio = data_cfg.get("train_ratio", 0.8) if isinstance(data_cfg, dict) else 0.8

    _, test_loader = load_data(
        csv_path, batch_size=batch_size, train_ratio=train_ratio, seed=seed
    )
...
def train():
    ...
    if isinstance(data_cfg, dict):
        csv_path = data_cfg.get("csv_path", "data/iris.csv")
        train_ratio = data_cfg.get("train_ratio", 0.8)
        data_folder = os.path.dirname(csv_path)
        download = False
    else:
        csv_path = str(data_cfg)
        train_ratio = 0.8
        data_folder = os.path.dirname(csv_path)
        download = True
    ...
    # ----- 保留旧行为：保存最终模型 -----
    torch.save(model.state_dict(), os.path.join(root_dir, "iris.pth"))
    print("训练完成，模型已保存。")
