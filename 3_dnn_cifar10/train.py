...
# 数据加载
# ===========================
def load_data(data_folder, batch_size=64, download=True, mirror_urls=None):
    if mirror_urls:
        datasets.CIFAR10.mirrors = mirror_urls
...
# 测试（独立模式，加载模型文件）
# ===========================
def test_from_file(model_path, data_folder="./data", batch_size=64):
    device = resolve_device("auto")
    model = Cifar10Network().to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))

    _, test_loader = load_data(data_folder, batch_size=batch_size, download=False)
...
def train():
    ...
    if isinstance(data_cfg, dict):
        data_folder = data_cfg.get("folder", "./data")
    ...
    # ----- 数据 -----
    mirror_urls = data_cfg.get("mirrors") if isinstance(data_cfg, dict) else None
    train_loader, test_loader = load_data(
        data_folder, batch_size=batch_size, download=download,
    ...
    # ----- 保留旧行为：保存最终模型 -----
    torch.save(model.state_dict(), os.path.join(root_dir, "cifar10.pth"))
    print("训练完成，模型已保存。")
