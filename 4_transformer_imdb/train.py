...
from datasets.imdb_dataset import get_loaders
...
MAX_LEN = 256
VOCAB_SIZE = 25000
MODEL_PATH = "checkpoints/transformer_imdb.pth"
...
def train():
    train_loader, test_loader, vocab_size = load_data(data_dir, batch_size=BATCH_SIZE)
...
def test(model=None, test_loader=None, model_path=MODEL_PATH):
    if model is None:
        if test_loader is None:
            _, test_loader, vocab_size = load_data(data_dir, batch_size=BATCH_SIZE)
        model = TransformerClassifier(vocab_size=vocab_size).to(device)
        model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()

    if test_loader is None:
        _, test_loader, _ = load_data(data_dir, batch_size=BATCH_SIZE)
